...
export async function getProductSubscribers(): Promise<number[]> {
  const { data } = await api.get('/product/count');
...
export function useProductSubscribers(options?: any) {
  return useQuery(['productSubscribers'], () => getProductSubscribers(), {
